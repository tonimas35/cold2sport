...
function FooterColumn({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div>
      <h3 className="mb-5 text-[11px] font-bold uppercase tracking-[0.2em] text-white/40">
        {title}
      </h3>
      <ul className="space-y-3">
        {links.map((link) => (
          <li key={link.label}>
            <Link
              href={link.href}
              className="text-sm text-gray-500 transition-colors duration-300 hover:text-brand-blue"
            >
              {link.label}
            </Link>
          </li>
...
function SocialIcon({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-gray-500 transition-all duration-300 hover:border-brand-blue/40 hover:text-brand-blue hover:shadow-[0_0_15px_rgba(0,180,255,0.15)]"
    >
      {children}
    </a>
  )
...
export default function Footer() {
  return (
    <footer className="relative bg-brand-darker" role="contentinfo">
      ...
      <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
        <div className="grid grid-cols-2 gap-12 md:grid-cols-4">
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="mt-3 text-sm text-gray-500">Elite Recovery Technology</p>
        ...
        <div className="mt-8 text-center text-[11px] tracking-wider text-gray-600">
